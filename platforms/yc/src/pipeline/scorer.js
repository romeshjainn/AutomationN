// ─────────────────────────────────────────────────────────────
//  platforms/yc/src/pipeline/scorer.js
//  100-point static scorer for YC jobs — AI fallback
//  Keywords weighted higher (startups value exact stack fit)
// ─────────────────────────────────────────────────────────────
...
export function scoreJob(job, keywords) {
  ...
  else recencyScore = FILTERS.RECENCY_SCORE.older ?? 3;

  // ── 3. Applicants score (0 → 20) ─────────────────────────────
  ...
  else if (ap < 100) applicantsScore = FILTERS.APPLICANTS_SCORE.under_100 ?? 5;
  else applicantsScore = FILTERS.APPLICANTS_SCORE.over_100 ?? 0;

  // ── 4. Experience score (0 → 15) ─────────────────────────────
