...
/**
 * Returns true if the job title matches any blocklist entry.
 */
export function isBlocked(title) {
  const lower = title.toLowerCase();
  return CONFIG.TITLE_BLOCKLIST.some((b) => lower.includes(b));
}

/**
 * Score a job out of 10 based on keyword matches in title + skills.
 */
export function scoreJob(title, skills, keywords) {
